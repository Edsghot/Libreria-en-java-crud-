...
public class Libreria {
    ...
    //HashMap
    public boolean rentarDocumento(int codigo) {
        Documento d = buscar(codigo);
        
        if(!listaPrestados.containsKey(d) && d instanceof Rentable) {//En este caso la llame le pasamos que seria del tipo de documento,el constrinsKey(Es una metodo Boolean)
            listaPrestados.put(d, (Rentable) d);//Si ya esta contenida,la agrega a mi lista de prestados pasandole la llave y el valor de tipo rentable
            return true;           
        }
        return false;
    }
    ...
    public void suscribirse(int dni) {
        ...
        if(persona != null && !personasSuscritas.contains(persona) && persona.puedeSuscribirse()) {
            personasSuscritas.add(persona);
            JOptionPane.showMessageDialog(null, "Ha sido agregado la persona con dni " + dni);
        }else {
            JOptionPane.showMessageDialog(null, "No puede suscribirse: " + persona);
        }
    }
